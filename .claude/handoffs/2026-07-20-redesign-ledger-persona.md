...
Pivot session: (1) rewrite the fact-check page as the site's single fact source, (2) implement the Claude Design handoff package (design + voice + interactions) across every page, (3) make public-action paths and the address lookup accurate and working, (4) create a project persona.
...
- **PR #6 — the redesign.** Modernist/RVC system (`#f3f2f2`/`#201e1d`/green `#1e6b3d`/gold `#c5a44e`, Archivo, zero radius, flush-left buttons) across all 12 pages. New landing `/` = the brief + an **action kit** with Census-geocoder lookup, public-action paths, and copy controls. Old print deck → `/deck.html`; `/brief-2026-08` 301s home. Signup backend is real: `site/_worker.js` → KV `SIGNUPS` (id `55371b2ca075430faeeae249f9b036cc`), bound via repo-root `wrangler.toml`; deploy is now just `npx wrangler pages deploy`.
...
- Official representation and legislative attribution were corrected against authoritative public pages. The current facts ledger records the verified details.
- Nassau county legislative coverage for RVC is verified against the official 2026 map.
...
- The action kit follows the appropriate official public-action route and documents exceptions without retaining recipient-specific routing here.
...
3. **Future public-action work:** before publishing or initiating any direct contact, same-day re-verify the relevant public claims and official status; run ground-claims.
